...
const addClient = () => {
  clientSections.value.push({ ...clientsO });
}

const removeClient = (index) => {
  clientSections.value.splice(index, 1)
}

const addInstitution = () => {
  institutionsSection.value.push({ ...institutions });
}

const removeInstitution = (index) => {
  institutionsSection.value.splice(index, 1)
}
...
onMounted(() => {
  axios.get(`${import.meta.env.VITE_SERVER}/transparency`,
    {
      headers: {
    ...
    .then(response => {
      ...
      institutionsSection.value = response.data.institutional.map(inst => { return { name: inst.name, text: inst.text } })
      clientSections.value = response.data.clients.map(client => { return { name: client.name, text: client.text } })
...
function onSubmit() {
  ...
  clientSections.value.forEach((client) => {
    formData.append("clientsName", client.name)

    formData.append("clientsText", client.text)
  });
  institutionsSection.value.forEach((inst) => {
    formData.append("institutionalText", inst.text)
    formData.append("institutionalName", inst.name)
  });
  ...
  axios.post(`${import.meta.env.VITE_SERVER}/transparency`,
    formData,
    {
      headers: {
    ...
    .then(function (response) {
      ...
      Swal.fire({
        icon: 'success',
        text: "Se editó la seccion de transparencia correctamente"
      })
      // location.reload()
      console.log(data);
    })
        ...
        <div class="ps-3">
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0 p-0">
              ...
              <li class="breadcrumb-item active" aria-current="page">Editar Transparencia</li>
            </ol>
          </nav>
        </div>
      </div>
          ...
          <div class="col-12 col-lg-12">
            <div class="card">
              <div class="card-body p-4">
                <h5 class="mb-4">Editar Transparencia</h5>
                <div class="row mb-3" v-for="(client, index) in clientSections" :key="index">
                  <label for="form-label" class="col-sm-12 col-form-label">Cliente</label>
                  <label for="form-label" class="col-sm-3 col-form-label">Titulo</label>
                  <div class="col-sm-9">
                  ...
                  <div class="d-md-flex d-grid align-items-center gap-3">
                    ...
                    <button type="button" class="btn btn-light px-4" @click="removeClient(index)">Quitar
                      elemento</button>
                  </div>
                </div>
                <div class="row mb-3" v-for="(insti, index) in institutionsSection" :key="index">
                  <label for="form-label" class="col-sm-12 col-form-label">Institucional</label>
                  <label for="form-label" class="col-sm-3 col-form-label">Titulo</label>
                  <div class="col-sm-9">
                    <input type="text" class="form-control" name="name" v-model="insti.name">
                  </div>
                  <label for="input37" class="col-sm-3 col-form-label">Texto</label>
                  <div class="col-sm-9">
                  ...
                  <div class="d-md-flex d-grid align-items-center gap-3">
                    ...
                    <button type="button" class="btn btn-light px-4" @click="removeInstitution(index)">Quitar
                      elemento</button>
                  </div>
                </div>
              </div>

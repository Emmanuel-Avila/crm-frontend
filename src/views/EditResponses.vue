...
onMounted(async () => {
  try {
    axios.get(`${import.meta.env.VITE_SERVER}/complaints/responses`, {
      headers: {
...
function updateResponses() {
  ...
  axios.post(`${import.meta.env.VITE_SERVER}/complaints/update-response`, data, {
    headers: {
          ...
          <div class="ps-3">
            <nav aria-label="breadcrumb">
              <ol class="breadcrumb mb-0 p-0">
                ...
                <li class="breadcrumb-item active" aria-current="page">Editar Respuestas</li>
              </ol>
            </nav>
          </div>
        </div>
                ...
                <form class="row g-3">
                  <div class="col-md-12">
                    ...
                    <select id="input19" class="form-select" v-model="response">
                      ...
                      <option v-for="(resp, index) in responses" :key='resp._id' :value="resp">{{ "Respuesta " +
                        (index + 1) }}</option>
                    </select>
                  </div>
                  <div class="col-md-12">
                    <label for="input23" class="form-label">Respuesta</label>
                    <textarea class="form-control" id="responseForm" rows="5"
                      name="responseForm">{{ response.text }}</textarea>
                  </div>
                  <div class="col-md-12">
                    <div class="d-md-flex d-grid align-items-center gap-3">
                      <button type="button" class="btn btn-primary px-4" @click="updateResponses">Guardar</button>
                    </div>
                  </div>
                </form>
              </div>

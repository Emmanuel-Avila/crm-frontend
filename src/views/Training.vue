...
onMounted(() => {
  axios.get(`${import.meta.env.VITE_SERVER}/training`, {
    headers: {
...
function onSubmit(values) {
  ...
  axios.post(`${import.meta.env.VITE_SERVER}/training`, data, {
    headers: {
      "Content-Type": "application/json",
      "Authorization": `Bearer ${localStorage.getItem("token")}`
    }
    ...
    .then(function (response) {
      ...
      Swal.fire({
        icon: 'success',
        text: "Se edito la capacitacion correctamente"
      })
    })
    .catch(function (error) {
      if (error.response.status === 401 || error.response.status === 403) {
          ...
          <div class="ps-3">
            <nav aria-label="breadcrumb">
              <ol class="breadcrumb mb-0 p-0">
                ...
                <li class="breadcrumb-item active" aria-current="page">Editar capacitacion</li>
              </ol>
            </nav>
          </div>
        </div>
            ...
            <Form @submit='onSubmit' :validation-schema="schema" v-slot="{ values }">
              <div class="card">
                <div class="card-body p-4">
                  <h5 class="mb-4">Formulario para capacitacion</h5>
                  <div class="row mb-3">

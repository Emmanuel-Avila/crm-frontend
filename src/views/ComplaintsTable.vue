...
const searchField = ["code", "names", "surnames", "email", "state", "complaintType"];
...
onMounted(async () => {
  try {
    axios.get(`${import.meta.env.VITE_SERVER}/complaints`, {
      headers: {
      ...
      .then(function (response) {
        ...
        complaints.value.forEach((comp) => comp.sentDate = formatDate(comp.sentDate))
        complaints.value.forEach((comp) => comp.department === " " ? comp.department = 'No especifico' : comp.department)
        complaints.value.forEach((comp) => comp.complaintType === "claim" ? comp.complaintType = "Reclamo" : comp.complaintType = "Queja")
        complaints.value.forEach((comp) => comp.state = formatState(comp.state))
        complaints.value.forEach((comp) => comp.office === " " ? comp.office = "No especifico" : comp.office)
...
watch(date, (newValue, oldValue) => {
  ...
  try {
    ...
    console.log(body)
    axios.post(`${import.meta.env.VITE_SERVER}/complaints/filter`, body, {
      headers: {
        "Authorization": `Bearer ${localStorage.getItem("token")}`
      }
    })
...
async function exportToExcel() {
  ...
  // Añadir encabezados
  worksheet.addRow(Object.keys(complaints.value[0]));
      ...
      <div class="main-content">
          ...
          <div class="ps-3">
            <nav aria-label="breadcrumb">
              <ol class="breadcrumb mb-0 p-0">
                ...
                <li class="breadcrumb-item active" aria-current="page">Tabla de Reclamos</li>
              </ol>
            </nav>
          </div>
        </div>
        ...
        <div class="card">
          <div class="card-body">
            ...
            <div class="row pb-2">
              <div class="col-sm-12">
                <button type="button" @click.stop="exportToExcel" class="btn btn-lg btn-success">
              ...
              <div class="col-sm-12 col-md-6">
                <div class="row">
                  <div class="col-sm-3 d-flex">
                    <label class="m-auto">Filtrar por Fecha:</label>
                  </div>
                  <div class="col-sm-9">
                    <VueDatePicker v-model="date" locale="es" :enable-time-picker="false" range></VueDatePicker>
                  </div>
                </div>
              </div>
              <div class="col-sm-12 col-md-6">
            ...
            <Vue3EasyDataTable buttons-pagination :headers="headers" :items="complaints" theme-color="#0d6efd"
              ...
              <template #header-complaintType="header">
                <div class="filter-column">
                  <img src="/assets/images/icon/filter.png" alt="" class="header-icon"
                    @click.stop="showTypeFilter = !showTypeFilter">
                  {{ header.text }}
                  <div v-if="showTypeFilter" class="filter-menu">
                    <select name="type" id="" class="form-control selector" v-model="typeCriteria">
              ...
              <template #header-department="header">
                <div class="filter-column">
                  <img src="/assets/images/icon/filter.png" alt="" class="header-icon"
                    @click.stop="showDepartmentFilter = !showDepartmentFilter">
                  {{ header.text }}
                  <div v-if="showDepartmentFilter" class="filter-menu">
                    <select name="type" id="" class="form-control selector" v-model="departmentCriteria">
                      <option value="all">Todos</option>
                      <option :value="depar.name" v-for="(depar, index) in departaments" :key="depar.id">{{ depar.name }}
                      </option>
                    </select>
                  </div>
                </div>
              </template>
              <template #header-office="header">
                <div class="filter-column">
                  <img src="/assets/images/icon/filter.png" alt="" class="header-icon"
                    @click.stop="showOfficeFilter = !showOfficeFilter">
                  {{ header.text }}
                  <div v-if="showOfficeFilter" class="filter-menu">
                    <select name="type" id="" class="form-control selector" v-model="officeCriteria">
                      <option value="all">Todos</option>
              ...
              <template #header-state="header">
                <div class="filter-column">
                  <img src="/assets/images/icon/filter.png" alt="" class="header-icon"
                    @click.stop="showStateFilter = !showStateFilter">
                  {{ header.text }}
                  <div v-if="showStateFilter" class="filter-menu">
                    <select name="state" id="" class="form-control selector" v-model="stateCriteria">
                      <option value="all">Todos</option>
